...
import ac.altarac.utils.data.packetentity.PacketEntitySizeable;
import ac.altarac.utils.data.packetentity.dragon.PacketEntityEnderDragonPart;
...
@CheckData(name = "Reach", stableKey = "AltarAC.combat.reach", description = "Attacked an entity from too far away", setback = 10)
public class Reach extends Check implements PacketCheck {
    ...
    public Reach(AltarACPlayer player) {
        super(player);
    }

    @Override
    public void onPacketReceive(final PacketReceiveEvent event) {
        if (!player.disablePlugin && event.getPacketType() == PacketType.Play.Client.ATTACK) {
        ...
        if (!player.disablePlugin && event.getPacketType() == PacketType.Play.Client.INTERACT_ENTITY) {
    ...
    private void onInteract(PacketReceiveEvent event, int entityId, InteractionHand hand) {
        ...
        // Dead entities cause false flags (https://github.com/AltarAC/AltarAC/issues/546)
