package ac.altarac.checks.impl.aim.processor;
...
import ac.altarac.utils.lists.RunningMode;
...
public class AimProcessor extends Check implements RotationCheck {
    ...
    public AimProcessor(AltarACPlayer playerData) {
        super(playerData);
    }
    ...
    public void process(final RotationUpdate rotationUpdate) {
        ...
        this.divisorX = AltarACMath.gcd(deltaXRot, lastXRot);
        if (deltaXRot > 0 && deltaXRot < 5 && divisorX > AltarACMath.MINIMUM_DIVISOR) {
        ...
        this.divisorY = AltarACMath.gcd(deltaYRot, lastYRot);

        if (deltaYRot > 0 && deltaYRot < 5 && divisorY > AltarACMath.MINIMUM_DIVISOR) {
