...
public class TickBasedMovingBorderExtent implements BorderExtent {
    ...
    private double calculateSize() {
        ...
        return progress < 1.0D ? AltarACMath.lerp(progress, from, to) : to;
    }
    ...
    public double getMinX(double centerX, double absoluteMaxSize) {
        return AltarACMath.clamp(centerX - previousSize / 2.0, -absoluteMaxSize, absoluteMaxSize);
    }
    ...
    public double getMinZ(double centerZ, double absoluteMaxSize) {
        return AltarACMath.clamp(centerZ - previousSize / 2.0, -absoluteMaxSize, absoluteMaxSize);
    }
