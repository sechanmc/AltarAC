package ac.altarac.utils.anticheat.update;

import ac.altarac.player.AltarACPlayer;
import ac.altarac.utils.anticheat.LogUtil;
import ac.altarac.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(AltarACPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable HitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = AltarACMath.radians(player.pitch);
        float yaw = AltarACMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[AltarACMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
