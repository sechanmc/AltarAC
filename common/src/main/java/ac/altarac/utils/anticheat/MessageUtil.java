...
import ac.altarac.utils.data.webhook.discord.CompiledDiscordTemplate;
...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable AltarACPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable AltarACPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : AltarACAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable AltarACPlayer AltarACPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return AltarACAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = AltarACAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<AltarACUser, String>> variableReplacements = AltarACAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        return AltarACAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, grimReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull AltarACPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", AltarACAPI.INSTANCE.getConfigManager().getPrefix());
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = AltarACAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
