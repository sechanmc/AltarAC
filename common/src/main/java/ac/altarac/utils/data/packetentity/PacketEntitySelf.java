...
import ac.altarac.checks.impl.sprint.SprintD;
...
public class PacketEntitySelf extends PacketEntity {
    ...
    public PacketEntitySelf(AltarACPlayer player) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    }

    public PacketEntitySelf(AltarACPlayer player, PacketEntitySelf old) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    ...
    protected void initAttributes(AltarACPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.SNEAKING_SPEED, 0.3, 0, 1)
                .withGetRewriter(value -> {
                    ...
                    final double clamped = AltarACMath.clamp(0.3f + swiftSneak * 0.15f, 0f, 1f);
                    if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) {
                        return clamped;
                    }
