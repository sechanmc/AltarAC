package ac.altarac.utils.data.webhook.discord;
...
public record CompiledDiscordTemplate(Segment[] segments) {
    ...
    /**
     * @param player              AltarAC player (nullable)
     ...
     */
    public String render(@NotNull AltarACPlayer player,
                         @NotNull Map<String, String> statics,
                         @NotNull Map<String, Function<AltarACUser, String>> dynamics,
        ...
        for (Segment seg : segments) {
            if (seg instanceof Literal l) {
                ...
                if (val == null) {
                    Function<AltarACUser, String> fn = dynamics.get(p.key);
                    if (fn != null) val = fn.apply(player);
                }

                if (val == null) {
                    String resolved = AltarACAPI.INSTANCE.getMessagePlaceHolderManager()
