...
public class LatencyUtils {
    ...
    public LatencyUtils(AltarACPlayer player) {
        this.player = player;
    }
        ...
        synchronized (this) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonAltarACArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, AltarACAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
