package ac.altarac.utils.collisions.blocks.connecting;
...
public class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    public CollisionBox fetch(AltarACPlayer player, StateType heldItem, ClientVersion version, WrappedBlockState state, boolean isTargetBlock, int x, int y, int z) {
    ...
    private int[] getConnections(AltarACPlayer player, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
    ...
    @Override
    public boolean checkCanConnect(AltarACPlayer player, WrappedBlockState state, StateType one, StateType two, BlockFace direction) {
