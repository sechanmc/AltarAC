...
import ac.altarac.utils.data.MainSupportingBlockData;
...
import ac.altarac.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float blockFriction, AltarACPlayer player) {
     ...
     */
    public static StateType getOnPos(AltarACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(AltarACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(AltarACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(AltarACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(AltarACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(AltarACMath.floor(playerPos.getX()), AltarACMath.floor(playerPos.getY() - 0.5000001), AltarACMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(AltarACPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(AltarACMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(AltarACMath.floor(playerPos.getX()), AltarACMath.floor(playerPos.getY() - searchBelowPlayer), AltarACMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(AltarACPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(AltarACPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(AltarACMath.floor(x), AltarACMath.floor(y - 0.2F), AltarACMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(AltarACMath.floor(x), AltarACMath.floor(y - 1.2F), AltarACMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(AltarACPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(AltarACPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(AltarACPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) AltarACMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
