...
public class WorldRayTrace {
    public static HitData getNearestBlockHitResult(AltarACPlayer player, StateType heldItem, boolean sourcesHaveHitbox, boolean fluidPlacement, boolean itemUsePlacement) {
    ...
    public static HitData traverseBlocks(AltarACPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = AltarACMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = AltarACMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = AltarACMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = AltarACMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = AltarACMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = AltarACMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = AltarACMath.floor(startX);
        int floorStartY = AltarACMath.floor(startY);
        int floorStartZ = AltarACMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - AltarACMath.frac(startX) : AltarACMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - AltarACMath.frac(startY) : AltarACMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - AltarACMath.frac(startZ) : AltarACMath.frac(startZ));
