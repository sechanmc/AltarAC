...
import ac.altarac.events.packets.PacketWorldBorder;
import ac.altarac.player.AltarACPlayer;
import ac.altarac.predictionengine.blockeffects.BlockEffectsResolver;
import ac.altarac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_10;
import ac.altarac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_2;
import ac.altarac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_4;
import ac.altarac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_5;
import ac.altarac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_6;
import ac.altarac.utils.chunks.Column;
...
import ac.altarac.utils.data.tags.SyncedTags;
import ac.altarac.utils.functions.BlockAndPositionConsumer;
import ac.altarac.utils.functions.BlockAndPositionPredicate;
...
public final class Collisions {
    ...
    public static boolean slowCouldPointThreeHitGround(AltarACPlayer player, double x, double y, double z) {
    ...
    public static Vector3dm collide(AltarACPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector3dm collide(AltarACPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(AltarACPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean getCollisionBoxes(AltarACPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean isEmpty(AltarACPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector3dm maybeBackOffFromEdge(Vector3dm vec3, AltarACPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(AltarACPlayer player) {
    ...
    public static void handleInsideBlocks(AltarACPlayer player) {
        ...
        int minX = AltarACMath.mojangFloor(box.minX);
        int minY = AltarACMath.mojangFloor(box.minY);
        int minZ = AltarACMath.mojangFloor(box.minZ);
        int maxX = AltarACMath.mojangFloor(box.maxX);
        int maxY = AltarACMath.mojangFloor(box.maxY);
        int maxZ = AltarACMath.mojangFloor(box.maxZ);
    ...
    public static void onInsideBlock(AltarACPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, StateType blockType, WrappedBlockState block, int blockX, int blockY, int blockZ, boolean magic) {
    ...
    public static void applyEffectsFromBlocks(AltarACPlayer player) {
        ...
        player.stuckSpeedMultiplier = AltarACPlayer.DEFAULT_STUCK_SPEED;
        ...
        if (clientVersion.isOlderThan(ClientVersion.V_1_21_5)) {
            player.finalMovementsThisTick.add(new AltarACPlayer.Movement(from, to));
            ...
            if (player.finalMovementsThisTick.isEmpty()) {
                ...
                player.finalMovementsThisTick.add(new AltarACPlayer.Movement(player.finalMovementsThisTick.get(player.finalMovementsThisTick.size() - 1).to(), to));
            }
        }
    ...
    public static void resolveBlockEffects(AltarACPlayer player, Vector3d from, Vector3d to) {
        Collisions.resolveBlockEffects(player, List.of(new AltarACPlayer.Movement(from, to)));
    }

    public static void resolveBlockEffects(AltarACPlayer player, List<AltarACPlayer.Movement> movements) {
        resolveBlockEffects(player, player.clientVelocity, false, movements);
    }

    public static void resolveBlockEffects(AltarACPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, List<AltarACPlayer.Movement> movements) {
    ...
    private static double getOldDeltaY(AltarACPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? value / 0.98F + 0.08 : value;
    }

    private static double getNewDeltaY(AltarACPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? (value - 0.08) * 0.98F : value;
    }

    private static boolean isSlidingDown(Vector3dm vector, AltarACPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(AltarACPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(AltarACPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(AltarACPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getBlock(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(AltarACPlayer player, SimpleCollisionBox checkBox, @NotNull BlockAndPositionPredicate searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(@NotNull AltarACPlayer player, @NotNull SimpleCollisionBox checkBox, @NotNull BlockAndPositionConsumer searchingFor) {
    ...
    public static boolean onClimbable(AltarACPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(AltarACPlayer player, double x, double y, double z, WrappedBlockState blockData) {
