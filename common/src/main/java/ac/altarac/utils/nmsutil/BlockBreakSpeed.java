...
import ac.altarac.utils.data.tags.SyncedTag;
import ac.altarac.utils.data.tags.SyncedTags;
import ac.altarac.utils.enums.FluidTag;
import ac.altarac.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/AltarAC/AltarAC/issues/2117
    ...
    // another temporary hardcode for the same reasons as above; see https://github.com/AltarAC/AltarAC/issues/2574
    ...
    public static double getBlockDamage(AltarACPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(AltarACPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
        ...
        final boolean canHarvest = !block.isRequiresCorrectTool() || toolSpeedData.isCorrectToolForDrop
                // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/AltarAC/AltarAC/issues/2091
                || player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_4) && HARVESTABLE_TYPES_1_21_4.contains(block)
                // same reason as above; see https://github.com/AltarAC/AltarAC/issues/2574
    ...
    private static float getSpeedMultiplierFromToolData(AltarACPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(AltarACPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(AltarACPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
