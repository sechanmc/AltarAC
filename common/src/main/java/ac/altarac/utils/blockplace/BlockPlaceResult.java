...
import ac.altarac.utils.blockstate.helper.BlockFaceHelper;
...
import ac.altarac.utils.nmsutil.Dripstone;
...
public enum BlockPlaceResult {
    ...
    TORCH_OR_HEAD((player, place) -> {
        // type doesn't matter to AltarAC, same hitbox.
    ...
    // As long as one of these properties is met, it is good enough for AltarAC!
    VINE((player, place) -> {
    ...
    SCAFFOLDING((player, place) -> {
        ...
        } // else, cancel if the scaffolding is exactly 7 away, AltarAC doesn't handle this edge case yet.
