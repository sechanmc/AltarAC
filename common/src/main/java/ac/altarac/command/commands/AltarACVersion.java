...
public class AltarACVersion implements BuildableCommand {

    private static final AtomicReference<Component> updateMessage = new AtomicReference<>();
    private static final HttpClient HTTP_CLIENT = HttpClient.newBuilder()
            .connectTimeout(Duration.of(CommonAltarACArguments.URL_TIMEOUT.value(), ChronoUnit.MILLIS))
            .build();
    private static long lastCheck;

    public static void checkForUpdatesAsync(Sender sender) {
        String current = AltarACAPI.INSTANCE.getExternalAPI().getVersion();
        sender.sendMessage(Component.text()
                .append(Component.text("AltarAC Version: ").color(NamedTextColor.GRAY))
        ...
        lastCheck = now;
        AltarACAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(AltarACAPI.INSTANCE.getPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(CommonAltarACArguments.API_URL.value() + "updates"))
                    .GET()
                    .header("User-Agent", "AltarAC/" + AltarACAPI.INSTANCE.getExternalAPI().getVersion())
                    .header("Content-Type", "application/json")
                    .timeout(Duration.of(CommonAltarACArguments.URL_TIMEOUT.value(), ChronoUnit.MILLIS))
                    .build();
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest AltarAC version. Update server responded with status code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(AltarACAPI.INSTANCE.getExternalAPI().getVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of AltarAC").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of AltarAC").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New AltarAC version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown AltarAC version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest AltarAC version.", e);
        }
    }
