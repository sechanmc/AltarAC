...
import ac.altarac.command.CommandUtils;
...
public class AltarACList implements BuildableCommand {

    // Mainly for debugging purposes. Useful for seeing which players are exempt or not.

    @Override
    public void register(CommandManager<Sender> commandManager, CloudPlatformCommandArguments arguments) {
        commandManager.command(commandManager.commandBuilder("AltarAC", "AltarAC")
                .literal("list")
                .permission("AltarAC.list")
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = AltarACAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
                .stream().collect(Collectors.toMap(AltarACIdentity::getUniqueId, Function.identity()));
        //
        ...
        //
        for (AltarACPlayer entry : AltarACAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !AltarACAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
