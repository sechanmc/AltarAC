...
public class AltarACDump implements BuildableCommand {
    ...
    private void handleDump(@NotNull CommandContext<Sender> context) {
        ...
        if (link != null) {
            sender.sendMessage(MessageUtil.miniMessage(AltarACAPI.INSTANCE.getConfigManager().getConfig()
                    .getStringElse("upload-log", "%prefix% &fUploaded debug to: %url%")
                    .replace("%url%", link)));
            return;
        }
        // TODO: change this back to application/json once allowed
        AltarACLog.sendLogAsync(sender, generateDump(), string -> link = string, "text/yaml");
    }
    ...
    public static JsonObject getDumpInfo() {
        ...
        versions.addProperty("AltarAC", AltarACAPI.INSTANCE.getExternalAPI().getVersion());
        ...
        versions.addProperty("implementation", AltarACAPI.INSTANCE.getPlatformServer().getPlatformImplementationString());
        // state of different properties
        JsonObject states = new JsonObject();
        base.add("states", states);
        if (AltarACAPI.INSTANCE.isInitialized()) states.addProperty("platform", AltarACAPI.INSTANCE.getPlatform().toString());
        ...
        if (AltarACAPI.INSTANCE.getAlertManager().hasConsoleVerboseEnabled()) settings.addProperty("console_verbose", true);
        if (!AltarACAPI.INSTANCE.getAlertManager().hasConsoleAlertsEnabled()) settings.addProperty("console_alerts", false);
        ...
        for (PlatformPlugin plugin : AltarACAPI.INSTANCE.getPluginManager().getPlugins()) {
    ...
    private static JsonObject getBuildInfo() {
        ...
        try {
            Properties properties = PropertiesUtil.readProperties(AltarACAPI.INSTANCE.getClass(), "altarac.properties");
