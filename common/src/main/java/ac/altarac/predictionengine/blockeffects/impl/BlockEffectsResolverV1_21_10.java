package ac.altarac.predictionengine.blockeffects.impl;

import ac.altarac.player.AltarACPlayer;
import ac.altarac.predictionengine.blockeffects.BlockCollisions;
import ac.altarac.predictionengine.blockeffects.BlockEffectsResolver;
import ac.altarac.predictionengine.blockeffects.BlockStepVisitor;
...
public class BlockEffectsResolverV1_21_10 implements BlockEffectsResolver {
    ...
    @Override
    public void applyEffectsFromBlocks(AltarACPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, List<AltarACPlayer.Movement> movements) {
        LongSet visitedBlocks = player.visitedBlocks;

        for (AltarACPlayer.Movement movement : movements) {
    ...
    public static int checkInsideBlocks(AltarACPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, Vector3d from, Vector3d to, LongSet visitedBlocks, int count) {
        SimpleCollisionBox boundingBox = GetBoundingBox.getCollisionBoxForPlayer(player, to.x, to.y, to.z).expand(-1.0E-5F);
        boolean isFarEnough = from.distanceSquared(to) > AltarACMath.square(0.9999900000002526);
        ...
        forEachBlockIntersectedBetween(from, to, boundingBox, (blockPos, localCount) -> {
            ...
            if (visitedBlocks.add(AltarACMath.asLong(blockPos))) {
    ...
    public static boolean forEachBlockIntersectedBetween(Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor visitor) {
        ...
        if (direction.lengthSquared() < (double) AltarACMath.square(1.0E-5F)) {
            ...
            for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(boundingBox.move(direction.multiply(-1.0)), direction)) {
                ...
                alreadyVisited.add(AltarACMath.asLong(blockPos));
            }
            ...
            if (iterationCount < 0) {
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(boundingBox, direction)) {
                    if (alreadyVisited.add(AltarACMath.asLong(blockPos)) && !visitor.visit(blockPos, iterationCount + 1)) {
                        return false;
                    }
                }
    ...
    public static int addCollisionsAlongTravel(LongSet alreadyVisited, Vector3d direction, SimpleCollisionBox boundingBox, BlockStepVisitor visitor) {
        ...
        int currentX = AltarACMath.floor(start.x);
        int currentY = AltarACMath.floor(start.y);
        int currentZ = AltarACMath.floor(start.z);
        int stepX = AltarACMath.sign(direction.x);
        int stepY = AltarACMath.sign(direction.y);
        int stepZ = AltarACMath.sign(direction.z);
        ...
        double tDeltaX = tMaxX * (stepX > 0 ? 1.0 - AltarACMath.frac(start.x) : AltarACMath.frac(start.x));
        double tDeltaY = tMaxY * (stepY > 0 ? 1.0 - AltarACMath.frac(start.y) : AltarACMath.frac(start.y));
        double tDeltaZ = tMaxZ * (stepZ > 0 ? 1.0 - AltarACMath.frac(start.z) : AltarACMath.frac(start.z));
        ...
        while (tDeltaX <= 1.0 || tDeltaY <= 1.0 || tDeltaZ <= 1.0) {
            ...
            if (!collisionPoint.isEmpty()) {
                ...
                double clampedX = AltarACMath.clamp(collisionVec.x, currentX + 1.0E-5F, currentX + 1.0 - 1.0E-5F);
                double clampedY = AltarACMath.clamp(collisionVec.y, currentY + 1.0E-5F, currentY + 1.0 - 1.0E-5F);
                double clampedZ = AltarACMath.clamp(collisionVec.z, currentZ + 1.0E-5F, currentZ + 1.0 - 1.0E-5F);
                int endX = AltarACMath.floor(clampedX - sizeX * furthestCorner.getX());
                int endY = AltarACMath.floor(clampedY - sizeY * furthestCorner.getY());
                int endZ = AltarACMath.floor(clampedZ - sizeZ * furthestCorner.getZ());
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(currentX, currentY, currentZ, endX, endY, endZ, direction)) {
                    if (alreadyVisited.add(AltarACMath.asLong(blockPos)) && !visitor.visit(blockPos, copyIterationCount)) {
                        return -1;
                    }
                }
            }
