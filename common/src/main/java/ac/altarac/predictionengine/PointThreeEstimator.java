...
import ac.altarac.utils.data.VelocityData;
 ...
 * Someone trying to false AltarAC by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(AltarACPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/AltarAC/issues/613
    ...
    private double iterateGravity(AltarACPlayer player, double y) {
