...
import ac.altarac.checks.impl.vehicle.VehicleC;
import ac.altarac.checks.type.PositionCheck;
import ac.altarac.manager.SetbackTeleportUtil;
import ac.altarac.player.AltarACPlayer;
import ac.altarac.predictionengine.movementtick.MovementTickerCamel;
import ac.altarac.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.altarac.predictionengine.movementtick.MovementTickerHorse;
import ac.altarac.predictionengine.movementtick.MovementTickerNautilus;
import ac.altarac.predictionengine.movementtick.MovementTickerPig;
import ac.altarac.predictionengine.movementtick.MovementTickerPlayer;
import ac.altarac.predictionengine.movementtick.MovementTickerStrider;
import ac.altarac.predictionengine.predictions.PredictionEngineNormal;
import ac.altarac.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.altarac.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.altarac.utils.data.SetBackData;
import ac.altarac.utils.data.VectorData;
import ac.altarac.utils.data.packetentity.PacketEntity;
import ac.altarac.utils.data.packetentity.PacketEntityCamel;
...
import ac.altarac.utils.data.packetentity.PacketEntityTrackXRot;
...
import ac.altarac.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(AltarACPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in AltarAC
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, AltarACMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, AltarACMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, AltarACMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, AltarACMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on AltarAC.
                //
...
//                    player.uncertaintyHandler.isSteppingOnSlime = true;
        ...
        } else if (player.disablePlugin || (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_8) && player.gamemode == GameMode.SPECTATOR) || player.isFlying || (player.isExemptElytra() && player.isGliding)) {
