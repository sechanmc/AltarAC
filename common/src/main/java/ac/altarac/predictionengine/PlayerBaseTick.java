package ac.altarac.predictionengine;

import ac.altarac.player.AltarACPlayer;
import ac.altarac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.altarac.utils.data.attribute.ValuedAttribute;
...
import ac.altarac.utils.latency.CompensatedEntities;
...
import ac.altarac.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(AltarACPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(AltarACPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(AltarACPlayer player) {
    ...
    private static void updateFluidOnEyes(AltarACPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(AltarACPlayer player) {
    ...
    public static void updatePowderSnow(AltarACPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(AltarACPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(AltarACPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(AltarACPlayer player) {
    ...
    private static void moveTowardsClosestSpace(AltarACPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(AltarACPlayer player, double x, double z) {
        int floorX = AltarACMath.floor(x);
        int floorZ = AltarACMath.floor(z);
        int floorY = AltarACMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(AltarACPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(AltarACPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(AltarACPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(AltarACPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = AltarACMath.floor(aABB.minX);
        int ceilX = AltarACMath.ceil(aABB.maxX);
        int floorY = AltarACMath.floor(aABB.minY);
        int ceilY = AltarACMath.ceil(aABB.maxY);
        int floorZ = AltarACMath.floor(aABB.minZ);
        int ceilZ = AltarACMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(AltarACPlayer player, FluidTag tag, double multiplier) {
    ...
    public static boolean updateFluidInteraction(AltarACPlayer player) {
    ...
    private static boolean suffocatesAt(AltarACPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(AltarACPlayer player, int x, int y, int z) {
