...
import ac.altarac.predictionengine.PlayerBaseTick;
import ac.altarac.predictionengine.predictions.PredictionEngine;
import ac.altarac.predictionengine.predictions.PredictionEngineElytra;
...
import ac.altarac.utils.nmsutil.EntityTypeTags;
import ac.altarac.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.altarac.utils.nmsutil.GetBoundingBox;
import ac.altarac.utils.nmsutil.MainSupportingBlockPosFinder;
import ac.altarac.utils.viaversion.ViaVersionUtil;
import ac.altarac.utils.team.EntityPredicates;
import ac.altarac.utils.team.EntityTeam;
...
public class MovementTicker {
    public final AltarACPlayer player;

    public static void handleEntityCollisions(AltarACPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !AltarACMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !AltarACMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new AltarACPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(AltarACMath.floor(player.lastX), AltarACMath.floor(player.lastY), AltarACMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other AltarAC stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other AltarAC stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other AltarAC stuff
