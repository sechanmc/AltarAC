...
import ac.altarac.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(AltarACPlayer player) {
    ...
    private static BoatEntityStatus getStatus(AltarACPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull AltarACPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = AltarACMath.floor(box.minX);
        int maxX = AltarACMath.ceil(box.maxX);
        int minY = AltarACMath.floor(box.maxY);
        int maxY = AltarACMath.ceil(maxBoxY);
        int minZ = AltarACMath.floor(box.minZ);
        int maxZ = AltarACMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(AltarACPlayer AltarACPlayer) {
        SimpleCollisionBox box = AltarACPlayer.boundingBox;
        int minX = AltarACMath.floor(box.minX);
        int maxX = AltarACMath.ceil(box.maxX);
        int minY = AltarACMath.floor(box.minY);
        int maxY = AltarACMath.ceil(box.minY + 0.001D);
        int minZ = AltarACMath.floor(box.minZ);
        int maxZ = AltarACMath.ceil(box.maxZ);
        boolean inWater = false;
        AltarACPlayer.vehicleData.waterLevel = -Double.MAX_VALUE;

        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    double level = AltarACPlayer.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    if (level > 0) {
                        float f = (float) ((float) y + level);
                        AltarACPlayer.vehicleData.waterLevel = Math.max(f, AltarACPlayer.vehicleData.waterLevel);
    ...
    public static float getGroundFriction(AltarACPlayer player) {
    ...
    private void floatBoat(AltarACPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(AltarACPlayer player) {
    ...
    private void controlBoat(AltarACPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(AltarACMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(AltarACMath.radians(player.yaw)) * f)));
    }
}
