package ac.altarac.player;

import ac.altarac.AltarACAPI;
import ac.altarac.api.AbstractCheck;
import ac.altarac.api.AltarACUser;
import ac.altarac.api.PacketWorld;
import ac.altarac.api.config.ConfigManager;
import ac.altarac.api.handler.ResyncHandler;
import ac.altarac.checks.Check;
import ac.altarac.checks.impl.aim.processor.AimProcessor;
import ac.altarac.checks.impl.misc.ClientBrand;
import ac.altarac.checks.impl.misc.TransactionOrder;
import ac.altarac.checks.impl.packetorder.PacketOrderProcessor;
import ac.altarac.events.packets.CheckManagerListener;
import ac.altarac.manager.*;
import ac.altarac.manager.player.features.FeatureManagerImpl;
import ac.altarac.manager.player.handlers.DefaultResyncHandler;
import ac.altarac.manager.player.handlers.NoOpResyncHandler;
import ac.altarac.platform.api.player.PlatformPlayer;
import ac.altarac.predictionengine.EntityFluidInteraction;
import ac.altarac.predictionengine.MovementCheckRunner;
import ac.altarac.predictionengine.PointThreeEstimator;
import ac.altarac.predictionengine.UncertaintyHandler;
import ac.altarac.manager.AttackCooldownHandler;
...
import ac.altarac.utils.change.PlayerBlockHistory;
import ac.altarac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.altarac.utils.data.*;
...
import ac.altarac.utils.data.packetentity.PacketEntitySelf;
import ac.altarac.utils.data.tags.SyncedTags;
import ac.altarac.utils.enums.FluidTag;
import ac.altarac.utils.enums.Pose;
import ac.altarac.utils.latency.*;
import ac.altarac.utils.math.AltarACMath;
import ac.altarac.utils.math.Location;
import ac.altarac.utils.math.TrigHandler;
...
import ac.altarac.utils.nmsutil.GetBoundingBox;
import ac.altarac.utils.nmsutil.Materials;
...
public class AltarACPlayer implements AltarACUser {
    ...
    @Getter @Setter private ResyncHandler resyncHandler = AltarACAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    // This variable is for support with test servers that want to be able to disable AltarAC
    // AltarAC disabler 2022 still working!
    ...
    public AltarACPlayer(@NotNull User user) {
        this.user = user;
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, AltarACAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void sendTransaction(boolean async) {
        ...
        if (disablePlugin && (System.nanoTime() - getPlayerClockAtLeast()) > 15e9) {
            return;
        }
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, AltarACAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            AltarACAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, AltarACAPI.INSTANCE.getPlugin(),
    ...
    public void pollData() {
        ...
        if (!AltarACAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            AltarACAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = AltarACAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
        ...
        if (uuid != null) {
            AltarACAPI.INSTANCE.getDataStoreLifecycle().sessionTracker()
    ...
    public void updatePermissions() {
        runSafely(() -> {
            try {
                boolean noModifyPacketPermission = hasPermission("AltarAC.nomodifypacket");
                boolean noSetbackPermission = hasPermission("AltarAC.nosetback");
                boolean disabledPermission = hasPermission("AltarAC.disabled");
                boolean exemptPermission = hasPermission("AltarAC.exempt");
                ...
                this.disablePlugin = disabledPermission;
                if (exemptPermission) {
    ...
    public int getTransactionPing() {
        return AltarACMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = AltarACMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(AltarACMath.floor(box.minX), AltarACMath.floor(box.minY), AltarACMath.floor(box.minZ),
                AltarACMath.ceil(box.maxX), AltarACMath.ceil(box.maxY), AltarACMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(AltarACPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            AltarACPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            AltarACPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            AltarACPlayer.Movement movement3 = new AltarACPlayer.Movement(movement1.from(), movement2.to());
