...
import ac.altarac.checks.impl.aim.AimDuplicateLook;
import ac.altarac.checks.impl.aim.AimModulo360;
import ac.altarac.checks.impl.aim.processor.AimProcessor;
import ac.altarac.checks.impl.badpackets.*;
import ac.altarac.checks.impl.breaking.*;
import ac.altarac.checks.impl.chat.ChatA;
import ac.altarac.checks.impl.chat.ChatB;
import ac.altarac.checks.impl.chat.ChatC;
import ac.altarac.checks.impl.chat.ChatD;
import ac.altarac.checks.impl.combat.*;
import ac.altarac.checks.impl.crash.*;
import ac.altarac.checks.impl.elytra.*;
import ac.altarac.checks.impl.exploit.ExploitA;
import ac.altarac.checks.impl.exploit.ExploitB;
import ac.altarac.checks.impl.groundspoof.NoFall;
import ac.altarac.checks.impl.misc.ClientBrand;
import ac.altarac.checks.impl.misc.GhostBlockMitigation;
import ac.altarac.checks.impl.misc.Post;
import ac.altarac.checks.impl.misc.TransactionOrder;
import ac.altarac.checks.impl.movement.NoSlow;
import ac.altarac.checks.impl.movement.PredictionRunner;
import ac.altarac.checks.impl.movement.SetbackBlocker;
import ac.altarac.checks.impl.movement.VehiclePredictionRunner;
import ac.altarac.checks.impl.multiactions.*;
import ac.altarac.checks.impl.packetorder.*;
import ac.altarac.checks.impl.prediction.DebugHandler;
import ac.altarac.checks.impl.prediction.GroundSpoof;
import ac.altarac.checks.impl.prediction.OffsetHandler;
import ac.altarac.checks.impl.prediction.Phase;
import ac.altarac.checks.impl.scaffolding.*;
import ac.altarac.checks.impl.sprint.*;
import ac.altarac.checks.impl.timer.*;
import ac.altarac.checks.impl.vehicle.*;
import ac.altarac.checks.impl.velocity.ExplosionHandler;
import ac.altarac.checks.impl.velocity.KnockbackHandler;
import ac.altarac.checks.type.*;
import ac.altarac.events.packets.PacketChangeGameState;
import ac.altarac.events.packets.PacketEntityReplication;
import ac.altarac.events.packets.PacketPlayerAbilities;
...
import ac.altarac.predictionengine.GhostBlockDetector;
import ac.altarac.predictionengine.SneakingEstimator;
import ac.altarac.utils.anticheat.update.*;
import ac.altarac.utils.latency.CompensatedCameraEntity;
import ac.altarac.utils.latency.CompensatedCooldown;
import ac.altarac.utils.latency.CompensatedFireworks;
import ac.altarac.utils.latency.CompensatedInventory;
import ac.altarac.utils.team.TeamHandler;
...
public class CheckManager {
    ...
    public CheckManager(AltarACPlayer player) {
...
//                .put(PacketOrderP.class, new PacketOrderP(player))
    ...
    private void registerBuiltInVerboseTemplates() {
        VerboseRegistry registry = AltarACAPI.INSTANCE.getDataStoreLifecycle().verboseRegistry();
    ...
    private void init() {
        ...
        for (final AbstractCheck check : allChecks.values()) {
            ...
            for (String permissionName : permissions) {
                permissionName += id;
                AltarACAPI.INSTANCE.getPermissionManager().registerPermission(permissionName, PermissionDefaultValue.FALSE);
            }
        }
    }
}
