...
import ac.altarac.checks.impl.badpackets.BadPacketsN;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    private static final AltarACTeleportEvent.Channel TELEPORT_CHANNEL = AltarACAPI.INSTANCE.getEventBus().get(AltarACTeleportEvent.class);
    private static final AltarACPlayerSetbackEvent.Channel PLAYER_SETBACK_CHANNEL = AltarACAPI.INSTANCE.getEventBus().get(AltarACPlayerSetbackEvent.class);
    ...
    public SetbackTeleportUtil(AltarACPlayer player) {
        super(player);
    }
    ...
    public void executeForceResync() {
        if (player.gamemode == GameMode.SPECTATOR || player.disablePlugin)
    ...
    private boolean isExempt() {
        ...
        if (player.disablePlugin) return true;
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    AltarACAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, AltarACAPI.INSTANCE.getPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            AltarACEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if AltarAC ever supports 1.7 again
            }
            ...
            // Dual fire: packet-level signal for anticheat compat (AltarACTeleportEvent),
            // semantic signal for admin/observability consumers (AltarACPlayerSetbackEvent).
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(AltarACMath.floor(player.x) >> 4, AltarACMath.floor(player.z) >> 4);

        // If true, the player is in an unloaded chunk
        return !player.disablePlugin && (column == null || column.transaction() >= player.lastTransactionReceived.get() ||
