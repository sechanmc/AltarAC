...
import ac.altarac.manager.init.ReloadableInitable;
...
import ac.altarac.utils.data.webhook.discord.Embed;
import ac.altarac.utils.data.webhook.discord.EmbedField;
import ac.altarac.utils.data.webhook.discord.EmbedFooter;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    private static final Duration timeout = Duration.ofMillis(CommonAltarACArguments.URL_TIMEOUT.value());
    ...
    private static String validatedConfigURL(String configPath, String defaultURL) {
        String url = AltarACAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-image-url", defaultURL);
    ...
    public void reload() {
        try {
            ...
            ConfigManager config = AltarACAPI.INSTANCE.getConfigManager().getConfig();
            ...
            embedFooterUrl = validatedConfigURL("embed-footer-url", "https://AltarAC.ac/images/AltarAC.png");
            embedFooterText = config.getStringElse("embed-footer-text", "v%AltarAC_version%");
            embedTitle = config.getStringElse("embed-title", "**AltarAC Alert**");
    ...
    public void sendAlert(@NotNull AltarACPlayer player, String verbose, String checkName, int violations) {
        if (isDisabled()) {
            return;
        }

        // Per-alert overlay — avoids polluting the global static map
        Map<String, String> statics = new HashMap<>(AltarACAPI.INSTANCE.getExternalAPI().getStaticReplacements());
        statics.put("%check%", checkName);
        statics.put("%violations%", Integer.toString(violations));

        Map<String, Function<AltarACUser, String>> dynamics = AltarACAPI.INSTANCE.getExternalAPI().getVariableReplacements();
    ...
    public CompletableFuture<Boolean> sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            AltarACAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(AltarACAPI.INSTANCE.getPlugin(), DiscordManager::tick, 0, 1);
        }
