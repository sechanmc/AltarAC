...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = AltarACAPI.INSTANCE.getPlatformServer();
    ...
    /**
     * Gets the non-null PlatformPlayer from a AltarACUser.
     * @throws IllegalArgumentException if the user is not a AltarACPlayer.
     * @throws NullPointerException if the AltarACPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull AltarACUser user) {
        Objects.requireNonNull(user, "user cannot be null"); // Should be guaranteed by interface contract, but good practice

        if (!(user instanceof AltarACPlayer AltarACPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-AltarACPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = AltarACPlayer.platformPlayer;
    ...
    @Override
    public boolean hasAlertsEnabled(@NotNull AltarACUser player) {
        return hasAlertsEnabled(requirePlatformPlayerFromUser(player));
    }

    @Override
    public void setAlertsEnabled(@NotNull AltarACUser player, boolean enabled, boolean silent) {
        setAlertsEnabled(requirePlatformPlayerFromUser(player), enabled, silent);
    }

    @Override
    public boolean hasVerboseEnabled(@NotNull AltarACUser player) {
        return hasVerboseEnabled(requirePlatformPlayerFromUser(player));
    }

    @Override
    public void setVerboseEnabled(@NotNull AltarACUser player, boolean enabled, boolean silent) {
        setVerboseEnabled(requirePlatformPlayerFromUser(player), enabled, silent);
    }

    @Override
    public boolean hasBrandsEnabled(@NotNull AltarACUser player) {
        AltarACPlayer AltarACPlayer = (AltarACPlayer) player;
        ...
        if (AltarACPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(AltarACPlayer.platformPlayer);
    }

    @Override
    public void setBrandsEnabled(@NotNull AltarACUser player, boolean enabled, boolean silent) {
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(AltarACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(AltarACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(AltarACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(AltarACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
