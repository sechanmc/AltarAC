...
import ac.altarac.utils.blockplace.BlockPlaceResult;
import ac.altarac.utils.blockplace.ConsumesBlockPlace;
import ac.altarac.utils.change.BlockModification;
...
import ac.altarac.utils.nmsutil.*;
...
public class CheckManagerListener extends PacketListenerAbstract {
    ...
    private static void placeWaterLavaSnowBucket(AltarACPlayer player, ItemStack held, StateType toPlace, InteractionHand hand, int sequence) {
    ...
    public static void handleQueuedPlaces(AltarACPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    public static void handleQueuedBreaks(AltarACPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    private static void handleUseItem(AltarACPlayer player, ItemStack placedWith, InteractionHand hand, int sequence) {
    ...
    private static void handleBlockPlaceOrUseItem(PacketWrapper<?> packet, AltarACPlayer player) {
    ...
    private static void placeBucket(AltarACPlayer player, InteractionHand hand, int sequence) {
    ...
    public static void setPlayerItem(AltarACPlayer player, InteractionHand hand, ItemType type) {
    ...
    private static void placeLilypad(AltarACPlayer player, InteractionHand hand, int sequence) {
        HitData data = WorldRayTrace.getNearestBlockHitResult(player, null, true, false, true);
    ...
    private static boolean isMojangStupid(AltarACPlayer player, PacketReceiveEvent event, WrapperPlayClientPlayerFlying flying) {
        ...
        if (flying.hasPositionChanged() && flying.hasRotationChanged() &&
            ...
            // Additionally, only yaw/pitch matters: https://github.com/AltarAC/AltarAC/issues/1275#issuecomment-1872444018
    ...
    private static void handleFlying(AltarACPlayer player, double x, double y, double z, float yaw, float pitch, boolean hasPosition, boolean hasLook, boolean onGround, TeleportAcceptData teleportData, PacketReceiveEvent event) {
    ...
    private static void handleDigging(AltarACPlayer player, PacketReceiveEvent event) {
            ...
            if (damage >= 1) {
                ...
                player.blockHistory.add(
                        new BlockModification(
                                ...
                                blockBreak.position,
                                AltarACAPI.INSTANCE.getTickManager().currentTick,
