...
public final class RingRegistry {
                ...
                try {
                    exec.submit(() -> {
                        try { original.onEvent(event, sequence, endOfBatch); }
                            ...
                            "[AltarAC-datastore] single-writer handler failed", e); }
                    }).get(); // block until I/O completes
                } catch (Exception e) {
                    throw new ac.altarac.api.storage.backend.BackendException(
    ...
    private java.util.concurrent.ExecutorService singleWriterExecutor(@NotNull String backendId) {
        return singleWriterExecutors.computeIfAbsent(
                ...
                id -> java.util.concurrent.Executors.newSingleThreadExecutor(
                        namedDaemon("AltarAC-datastore-" + id + "-writer")));
    }
    ...
    private <E> void registerInternal(@NotNull Category<E> cat,
        ...
        ThreadFactory tf = namedDaemon("AltarAC-datastore-" + backendId + "-" + cat.id());
    ...
    private static <E> E resetIfKnown(E event) {
        ...
        if (event instanceof ac.altarac.api.storage.event.CheckCatalogEvent c) { c.reset(); return (E) c; }
        if (event instanceof ac.altarac.api.storage.event.ServerInstanceEvent i) { i.reset(); return (E) i; }
        if (event instanceof ac.altarac.api.storage.event.ServerStartupEvent s) { s.reset(); return (E) s; }
        if (event instanceof ac.altarac.api.storage.event.VerboseSchemaEvent v) { v.reset(); return (E) v; }
        if (event instanceof ac.altarac.api.storage.kind.CounterEvent<?> co) { co.clear(); return (E) co; }
        return event;
    }
    ...
    private long shutdownOne(Entry<?> e, long drainTimeoutMs) {
        try {
            ...
            logger.log(Level.WARNING,
                    "[AltarAC-datastore] ring " + e.category.id() + " shutdown timed out with "
