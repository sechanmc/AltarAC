package ac.altarac.internal.storage.backend.redis;
...
public final class RedisBackend implements Backend {
    ...
    public void init(@NotNull BackendContext ctx) throws BackendException {
        ...
        try {
            ...
            if (config.warnOnHistory()) {
                ...
                logger.warning("[AltarAC-datastore] Redis backend '"
    ...
    private void migrateV5ToV6(Jedis j) {
        logger.info("[AltarAC-datastore] migrating Redis violations id long→UUIDv7 …");
        ...
        logger.info("[AltarAC-datastore] migrated " + migrated + " violation rows to UUIDv7 id");
    }
    ...
    private void writeSession(SessionEvent s, long sequence, boolean endOfBatch) throws BackendException {
        ...
        try (Jedis j = pool.getResource()) {
            ...
            if (s.AltarACVersion() != null) h.put("AltarAC_version", s.AltarACVersion());
