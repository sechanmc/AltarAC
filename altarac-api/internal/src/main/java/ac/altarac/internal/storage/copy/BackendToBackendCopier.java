package ac.altarac.internal.storage.copy;
...
public final class BackendToBackendCopier {
     ...
     * {@link ac.altarac.internal.storage.backend.sqlite.SqliteBackend}
     * answers via a direct JDBC {@code SELECT uuid FROM AltarAC_players};
     * {@link ac.altarac.internal.storage.backend.memory.InMemoryBackend}
    ...
    private Set<UUID> collectPlayersBySessionWalk() throws BackendException {
        ...
        if (src instanceof ac.altarac.internal.storage.backend.sqlite.SqliteBackend sq) {
        ...
        if (src instanceof ac.altarac.internal.storage.backend.memory.InMemoryBackend mem) {
            ...
            synchronized (sq.writeMutexForCopier()) {
                ...
                try {
                    ...
                    try (java.sql.Statement s = conn.createStatement()) {
                        ac.altarac.api.storage.config.TableNames t = sq.tableNames();
