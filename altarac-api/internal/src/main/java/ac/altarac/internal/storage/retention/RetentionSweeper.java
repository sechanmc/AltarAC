package ac.altarac.internal.storage.retention;
    ...
    public void sweepOnce() {
        for (Map.Entry<Category<?>, RetentionRule> entry : rules.entrySet()) {
            ...
            try {
                ...
                logger.log(Level.WARNING, "[AltarAC-datastore] retention sweep interrupted");
                return;
            } catch (CompletionException | java.util.concurrent.ExecutionException | java.util.concurrent.TimeoutException e) {
                logger.log(Level.WARNING, "[AltarAC-datastore] retention sweep failed for " + cat.id(), e);
            }
        }
    }
}
