 ...
 * </ol>
 *
 * <p>Gated on {@code AltarAC_V2_SMOKE_DB} so the regular test suite stays
 * self-contained. Set the env var to the populated test database name
 * (e.g. {@code AltarAC_storage_test_19_4}) to exercise.
 */
...
class V2BackendBootstrapTest {
    ...
    void installAndDispatch() throws Exception {
        ...
        try (MongoClient probe = MongoClients.create(MONGO_CS)) {
            ...
            bindings.put(Categories.VIOLATION,
                new V2BackendBootstrap.Binding<>(StoreId.AltarAC("AltarAC_violations"), violationsKind));
            bindings.put(Categories.SESSION,
                new V2BackendBootstrap.Binding<>(StoreId.AltarAC("AltarAC_sessions"), sessionsKind));
            bindings.put(Categories.PLAYER_IDENTITY,
                new V2BackendBootstrap.Binding<>(StoreId.AltarAC("AltarAC_players"), playersKind));
    ...
    private static void verifyDispatch(V2Routes routes, MongoDatabase db,
        ...
        org.bson.Document anySession = db.getCollection("AltarAC_sessions").find().limit(1).first();
        ...
        org.bson.Document anyPlayer = db.getCollection("AltarAC_players").find().limit(1).first();
        if (anyPlayer != null) {
