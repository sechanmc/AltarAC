...
class V2RoutesBuilderTest {
    ...
    void duplicateRegistrationThrows() {
        ...
        b.register(Categories.SESSION, StoreId.AltarAC("sessions"), V2BuiltinKinds.sessions(), backend);

        // Second register with the same Category instance throws.
        IllegalStateException ex = assertThrows(IllegalStateException.class,
            () -> b.register(Categories.SESSION,
                StoreId.AltarAC("sessions_alt"), V2BuiltinKinds.sessions(), new StubBackend("backend-b")));
    ...
    void differentCategoriesCoexist() {
        ...
        V2Routes.Builder b = V2Routes.builder();
        b.register(Categories.SESSION, StoreId.AltarAC("sessions"), V2BuiltinKinds.sessions(), backend);
        b.register(Categories.PLAYER_IDENTITY, StoreId.AltarAC("players"), V2BuiltinKinds.players(), backend);
        ...
        assertTrue(sessionRoute.storeId().equals(StoreId.AltarAC("sessions")),
            "session route should target AltarAC:sessions, got " + sessionRoute.storeId());
        assertTrue(playerRoute.storeId().equals(StoreId.AltarAC("players")),
            "player route should target AltarAC:players, got " + playerRoute.storeId());
    }
    ...
    void identitySemanticsAllowDuplicateIds() {
        ...
        @SuppressWarnings({"unchecked", "rawtypes"})
        Category bErased = b;
        bld.register(aErased, StoreId.AltarAC("store_a"), V2BuiltinKinds.sessions(), backend);
        // Second register with .equals()-equal but distinct identity → does NOT collide.
        bld.register(bErased, StoreId.AltarAC("store_b"), V2BuiltinKinds.sessions(), backend);
        ...
        assertTrue(routeA != null && routeA.storeId().equals(StoreId.AltarAC("store_a")),
            "route for identity a must target AltarAC:store_a, got " + (routeA == null ? "null" : routeA.storeId()));
        assertTrue(routeB != null && routeB.storeId().equals(StoreId.AltarAC("store_b")),
            "route for identity b must target AltarAC:store_b, got " + (routeB == null ? "null" : routeB.storeId()));
    }
    ...
    private record SameIdCategory(@NotNull String customId) implements Category<Object> {
        ...
        public @NotNull ac.altarac.api.storage.category.AccessPattern accessPattern() {
            return ac.altarac.api.storage.category.AccessPattern.INDEXED_KV;
        }
    }
