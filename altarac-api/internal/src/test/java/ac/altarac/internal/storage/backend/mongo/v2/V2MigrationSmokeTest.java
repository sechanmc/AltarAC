package ac.altarac.internal.storage.backend.mongo.v2;
...
import ac.altarac.api.storage.kind.EventStream;
 ...
 * database. Targets the {@code AltarAC_storage_test_19_4} test fixture
 ...
 * </ol>
 *
 * <p>Gated on {@code AltarAC_V2_SMOKE_DB} system property — set the
 * environment variable to the database name (e.g.
 * {@code AltarAC_storage_test_19_4}) to exercise; skipped otherwise so
...
class V2MigrationSmokeTest {
    ...
    void smoke() throws Exception {
        String dbName = System.getProperty("AltarAC_V2_SMOKE_DB", System.getenv("AltarAC_V2_SMOKE_DB"));
        Assumptions.assumeTrue(dbName != null && !dbName.isEmpty(),
            "set AltarAC_V2_SMOKE_DB to the populated test database (e.g. AltarAC_storage_test_19_4)");
        ...
        try (MongoClient probeClient = MongoClients.create(MONGO_CS)) {
            ...
            long preSessionCount = db.getCollection("AltarAC_sessions").countDocuments();
            long preViolationCount = db.getCollection("AltarAC_violations").countDocuments();
            ...
            // legacy MongoBackend uses table-prefixed names (AltarAC_*), so
            ...
            // v2-native names.
            runMigrationsFor(backend, mctx, StoreId.AltarAC("AltarAC_violations"), V2BuiltinKinds.violations(), logger, failures);
            runMigrationsFor(backend, mctx, StoreId.AltarAC("AltarAC_sessions"),   V2BuiltinKinds.sessions(),   logger, failures);
            runMigrationsFor(backend, mctx, StoreId.AltarAC("AltarAC_players"),    V2BuiltinKinds.players(),    logger, failures);
            runMigrationsFor(backend, mctx, StoreId.AltarAC("AltarAC_checks"),     V2BuiltinKinds.checks(),     logger, failures);
            runMigrationsFor(backend, mctx, StoreId.AltarAC("server_instances"), V2BuiltinKinds.instances(), logger, failures);
            runMigrationsFor(backend, mctx, StoreId.AltarAC("server_startups"), V2BuiltinKinds.serverStartups(), logger, failures);
            ...
            org.bson.Document meta = db.getCollection("AltarAC_meta").find().first();
            assertNotNull(meta, "AltarAC_meta should exist post-migration");
    ...
    private static void verifySessionsReadable(MongoBackendV2 backend, MongoDatabase db, Logger logger) throws Exception {
        ...
        for (org.bson.Document raw : db.getCollection("AltarAC_sessions").find()) {
            ...
            Optional<SessionRecord> got = (Optional<SessionRecord>) adapter.execute(
                StoreId.AltarAC("AltarAC_sessions"), sessionsKind, op);
    ...
    private static void verifyPlayersReadable(MongoBackendV2 backend, MongoDatabase db, Logger logger) throws Exception {
        ...
        int read = 0;
        for (org.bson.Document raw : db.getCollection("AltarAC_players").find()) {
            ...
            Optional<PlayerIdentity> got = (Optional<PlayerIdentity>) adapter.execute(
                StoreId.AltarAC("AltarAC_players"), playersKind, op);
    ...
    private static void verifyChecksReadable(MongoBackendV2 backend, MongoDatabase db, Logger logger) throws Exception {
        ...
        int read = 0;
        for (org.bson.Document raw : db.getCollection("AltarAC_checks").find()) {
            ...
            Optional got = (Optional) adapter.execute(
                StoreId.AltarAC("AltarAC_checks"), checksKind, op);
