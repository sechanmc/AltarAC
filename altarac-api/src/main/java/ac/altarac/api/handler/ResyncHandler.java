package ac.altarac.api.handler;
 ...
 *   <li>Targeted block resyncs after cancelled block breaks flagged by AltarAC’s block breaking checks</li>
 *   <li>Packet-driven world updates in environments without backend world access (e.g. proxies)</li>
 * </ul>
 *
 * <p><strong>Default behavior:</strong></p>
 * AltarAC provides an internal fallback implementation that pulls block data from the platform's backend world
 ...
 *   <li>{@link #resyncPosition} is only used internally by AltarAC when a player fails or flags one of AltarAC’s
...
public interface ResyncHandler {
     ...
     * <p>
     * This method is only used internally by AltarAC when a player attempts to break a block
     * and fails a block break check (e.g., nuker, speed mine, reach). If the block break is
     * cancelled, AltarAC will call this method to restore the correct block on the client and
