...
public interface AltarACAbstractAPI extends ConfigReloadable, BasicReloadable {
    /**
     * Returns EventBus instanced used to register events and listen to AltarAC events
     * @return {@link EventBus}
     */
    @NotNull EventBus getEventBus();

    /**
     * Retrieves a AltarACUser reference from the player.
     * @param player Bukkit player reference
     * @return AltarACUser
     */
    @Nullable
    @Deprecated
    AltarACUser getUser(Player player);

    /**
     * Retrieves a AltarACUser reference from the player's UUID.
     * @param uuid UUID of the player
     * @return AltarACUser
     */
    @Nullable AltarACUser getUser(UUID uuid);
     ...
     * @param replacement
     */
    void registerVariable(String variable, @Nullable Function<AltarACUser, String> replacement);
    ...
    /**
     * Retrieves the plugin version of AltarAC.
     * @return AltarAC version
     */
    String getVersion();
    ...
    /**
     * Reloads AltarAC using the config file.
     */
    @Override
    default void reload() {
    ...
    /**
     * Reloads AltarAC asynchronously using the config file.
    ...
    /**
     * Resolves a platform-specific object into a {@link AltarACPlugin} wrapper.
     * <p>
     * This method is the bridge between platform-specific objects (like a Bukkit {@code JavaPlugin})
     * and the universal AltarAC API.
     * <p>
     ...
     * call this method once, cache the {@link AltarACPlugin} result, and pass that to the API instead.
     *
     * @param platformContext The platform-specific context (e.g., {@code this}).
     * @return The resolved AltarACPlugin wrapper.
     * @throws IllegalArgumentException If the provided context is not a valid plugin, mod, or class known to the platform.
     */
    @NotNull AltarACPlugin getPlugin(@NotNull Object platformContext);
     ...
     * already-built routing; register during plugin load, or before the AltarAC plugin finishes
     * enabling.
     */
