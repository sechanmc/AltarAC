...
public interface Backend {
     ...
     *       upgrading from a pre-v1 AltarAC, rows from the legacy
     *       {@code violations.sqlite} are replayed into the routed backend for
     *       the violation category so {@code /AltarAC history} is continuous
     *       across the upgrade. See {@link ac.altarac.api.storage.config.MigrationConfig}.</li>
     *   <li><strong>Cross-backend copy</strong> — {@code /AltarAC history copy}
    ...
    /**
     * Startup-time crash sweep: mark every session that's still "open"
     * (closed_at_epoch_ms IS NULL) as crashed by setting closed_at to the
     * row's last_activity_epoch_ms. Returns the number of sessions
     * affected.
     *
     ...
     * (e.g. a violation-only backend) skip the sweep silently.
     */
    default long markCrashedSessions() throws BackendException {
        return 0L;
    }
}
