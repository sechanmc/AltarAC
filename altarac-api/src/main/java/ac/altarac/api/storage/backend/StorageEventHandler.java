 ...
 * bundled copy.
 * <p>
 * {@code endOfBatch} is a hint to commit: batch work inside the handler
 * (prepared statements, in-memory buffers) and flush when the flag is
 * {@code true}. Handlers run on a single dedicated thread per category
 * ring; no external synchronisation is required.
 */
