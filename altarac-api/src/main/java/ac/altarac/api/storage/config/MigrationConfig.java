 ...
 * first boot after an operator upgrades from a pre-v1 AltarAC build.
 * <p>
 * V0 is the legacy history layout: a single {@code violations.sqlite} file
 * written by older AltarAC versions (pre-database-redesign), populated through
 ...
 * through the {@link ac.altarac.api.storage.DataStore} event ring and read
 * by {@link ac.altarac.api.storage.history.HistoryService}.
 * <p>
 * When an operator boots AltarAC with this feature for the first time and a V0
 * file is present in the plugin data folder, AltarAC walks the V0 rows and
 * bulk-imports them into the routed backend for the violation category so
 * {@code /AltarAC history} remains continuous across the upgrade. The migration
 * resumes from the last committed row if the server is restarted mid-run.
 * <p>
 * Once complete, the V0 file is left in place (read-only safety net — AltarAC
 ...
 *                       manually later via {@code /AltarAC history copy} or
