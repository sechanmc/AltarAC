...
/**
 * Base class for all AltarAC events.
 *
 ...
 * {@link EventBus#post(AltarACEvent)} / class-keyed {@code subscribe} methods
 ...
 */
public abstract class AltarACEvent<CHANNEL extends EventChannel<?, ?>> {
    private @Getter boolean cancelled = false;
    private final boolean async;

    protected AltarACEvent() {
        this(false); // Default to sync
    }

    protected AltarACEvent(boolean async) {
        this.async = async;
    }
     ...
     * should subscribe at a narrower level — {@code AltarACCheckEvent.Handler}
     * for cancellable check events, or the concrete event's own
     * {@code on…(...)} for full field access.
     */
    @FunctionalInterface
    public interface Handler {
        void onAnyEvent(@NotNull Class<? extends AltarACEvent<?>> eventClass, boolean currentlyCancelled);
    }

    public static final class Channel extends AbstractEventChannel<AltarACEvent<?>, Handler> {
        @SuppressWarnings({"unchecked", "rawtypes"})
        public Channel() {
            super((Class<AltarACEvent<?>>) (Class) AltarACEvent.class, Handler.class);
        }

        public void onAnyEvent(@NotNull AltarACPlugin plugin, @NotNull Handler handler) {
            subscribeAbstract(handler, 0, false, plugin);
        }

        public void onAnyEvent(@NotNull AltarACPlugin plugin, @NotNull Handler handler, int priority) {
            subscribeAbstract(handler, priority, false, plugin);
        }

        public void onAnyEvent(@NotNull AltarACPlugin plugin, @NotNull Handler handler, int priority, boolean ignoreCancelled) {
