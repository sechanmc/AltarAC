...
/**
 * Fired when AltarAC executes a setback against a player — the semantic
 * anticheat action, independent of the specific packet type used.
 *
 * <p>Dispatches through two concrete subtypes:
 * <ul>
 *   <li>{@link AltarACPlayerSetbackEvent} — player-on-foot setback, sent as a
 *       {@code ServerPlayerPositionAndLook} / teleport packet.</li>
 *   <li>{@link AltarACVehicleSetbackEvent} — player-in-vehicle setback, sent as
 ...
 */
public abstract class AltarACSetbackEvent<CHANNEL extends EventChannel<?, ?>> extends AltarACEvent<CHANNEL> {
    protected AltarACSetbackEvent() {
        super(true); // Async — setbacks are sent from the netty thread
    }

    /**
     * Abstract-level setback handler. Fires for every concrete
     * {@code AltarACSetbackEvent} subtype (player + vehicle) and any addon
    ...
    public interface Handler {
        void onAnySetback(@NotNull AltarACUser user, long timestamp);
    }

    public static final class Channel extends AbstractEventChannel<AltarACSetbackEvent<?>, Handler> {
        @SuppressWarnings({"unchecked", "rawtypes"})
        public Channel() {
            super((Class<AltarACSetbackEvent<?>>) (Class) AltarACSetbackEvent.class, Handler.class);
        }

        public void onAnySetback(@NotNull AltarACPlugin plugin, @NotNull Handler handler) {
            subscribeAbstract(handler, 0, false, plugin);
        }

        public void onAnySetback(@NotNull AltarACPlugin plugin, @NotNull Handler handler, int priority) {
