 ...
 * in a {@code AltarACEvent} object, and the legacy pooled-event pathway is
 ...
 *   <li><b>legacy</b> — holds a {@link AltarACEventListener} bound to the event's
 *       {@code AltarACEvent} subclass. Dispatched by populating a per-thread
 ...
 */
public abstract class EventChannel<E extends AltarACEvent<?>, H> {
     ...
     * Fabric {@code ModContainer}, …) into a {@link AltarACPlugin}. Used by the
     * deprecated {@code onX(Object ctx, Handler)} overloads on each
     * subclass so plugin authors that don't have a {@code AltarACPlugin} handy
     ...
     */
    @ApiStatus.Internal
    protected final @NotNull AltarACPlugin resolvePlugin(@NotNull Object pluginContext) {
        if (pluginContext instanceof AltarACPlugin) return (AltarACPlugin) pluginContext;
        PluginResolver r = this.pluginResolver;
        if (r == null) {
            throw new IllegalStateException("EventChannel " + eventClass.getSimpleName()
                    + " has no plugin resolver installed; pass a AltarACPlugin directly instead of a platform-specific context.");
        }
    ...
    public interface PluginResolver {
        @NotNull AltarACPlugin resolve(@NotNull Object pluginContext);
    }
    ...
    /**
     * Internal: registers a legacy {@link AltarACEventListener} on this channel's
     * legacy slot. Invoked by the bus when a caller uses the deprecated
     * class-keyed subscribe to bridge into the channel.
     */
    @ApiStatus.Internal
    public final <GE extends AltarACEvent<?>> void subscribeLegacy(
            @NotNull AltarACEventListener<GE> listener,
    ...
    @ApiStatus.Internal
    public final void unsubscribeLegacy(@NotNull AltarACEventListener<?> listener) {
    ...
    /**
     * Internal: dispatches a legacy-style {@code AltarACEvent} through this
     ...
     * via {@link #dispatchTypedFromLegacy(AltarACEvent, Object, boolean)}.
     *
     * <p>Only reachable through {@link EventBus#post(AltarACEvent)}. Internal
     * AltarAC firings should call the subclass's typed {@code fire(...)}
        ...
        for (Entry<H> e : arr) {
            ...
            try {
                if (e.legacyListener != null) {
                    ...
                    @SuppressWarnings("unchecked")
                    AltarACEventListener<E> listener = (AltarACEventListener<E>) e.legacyListener;
     ...
     * return {@code false}.
     *
     * <p>Only invoked via {@link #dispatchLegacy(AltarACEvent)}. The hot-path
    ...
    public static final class Entry<H> {
        ...
        public final @Nullable AltarACEventListener<? extends AltarACEvent<?>> legacyListener;
        public final @Nullable Class<? extends AltarACEvent<?>> legacyEventClass;
        ...
        Entry(@Nullable H handler,
              @Nullable AltarACEventListener<? extends AltarACEvent<?>> legacyListener,
              @Nullable Class<? extends AltarACEvent<?>> legacyEventClass,
              int priority, boolean ignoreCancelled,
              @Nullable Object pluginContext, @Nullable Class<?> declaringClass) {
        ...
        @SuppressWarnings("unchecked")
        public <GE extends AltarACEvent<?>> @NotNull AltarACEventListener<GE> legacyListenerAs() {
            return (AltarACEventListener<GE>) legacyListener;
        }
    }
}
